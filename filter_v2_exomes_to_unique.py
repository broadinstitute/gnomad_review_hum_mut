...
def main(args):
    ...
    v2_exome_liftover_ht = v2_exome_liftover_ht.key_by("original_locus", "original_alleles")
    v2_liftover_index = v2_exome_liftover_ht[ht.key]
    ht = ht.annotate(liftover_locus = v2_liftover_index.locus, liftover_alleles = v2_liftover_index.alleles)

    logger.info("Filtering variants not found in v2 genomes or v3 samples not found in v2 exomes...")
    idx = v2_genomes_ht.freq_meta.collect()[0].index({'group': 'adj'})
    ht = ht.annotate(v2_genomes_adj_freq = v2_genomes_ht[ht.locus,ht.alleles].freq[idx])
    v2_genomes_filtering_expr = (ht.v2_genomes_adj_freq.AC<1) | ~hl.is_defined(ht.v2_genomes_adj_freq.AC)
    ht = ht.filter(v2_genomes_filtering_expr)
    freq_adj_idx = v3_genomes_ht.freq_meta.collect()[0].index({'group': 'adj', 'subset': 'non_v2'})
    ht = ht.annotate(v3_non_v2_adj_freq = v3_genomes_ht[ht.liftover_locus,ht.liftover_alleles].freq[idx])
    v3_non_v2_filtering_expr = (ht.v3_non_v2_adj_freq.AC<1) | ~hl.is_defined(ht.v3_non_v2_adj_freq.AC)
    ...
    ht = ht.checkpoint(f"{args.output_path_prefix}/v2_exomes_unique_samples_with_variants.ht", overwrite = args.overwrite)
